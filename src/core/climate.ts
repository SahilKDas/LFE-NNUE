...
export function fillClimateRows(height:number,tick:number,temperature:Float32Array,fertility:Float32Array,gradient:Int8Array,band:Uint8Array):void { const wave=Math.sin(cyclePhase(tick)*Math.PI*2),step=2/Math.max(1,height-1),direction=Math.abs(wave*step)<.002?0:wave>0?-1:1;for(let y=0;y<height;y++){const latitude=1-2*y/Math.max(1,height-1),value=Math.max(0,Math.min(1,.5+.5*latitude*wave));temperature[y]=value;fertility[y]=.35+value;gradient[y]=direction;band[y]=Math.min(4,Math.floor(value*5));} }
