...
namespace {
class RenderWorkers {
  static constexpr int WorkerCount=9;
...
LRESULT CALLBACK procedure(HWND window, UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  if (message == PresentMessage) {if(uiBenchmark&&std::chrono::duration<double>(std::chrono::steady_clock::now()-uiBenchmarkStart).count()>=uiBenchmarkSeconds){writeUiBenchmarkReport();DestroyWindow(window);return 0;}scheduledPaint=true;InvalidateRect(window,nullptr,FALSE);UpdateWindow(window);return 0; }
  ...
  if (message == WM_TIMER) {
    ...
    lastRenderRequest=now;if(uiBenchmark&&std::chrono::duration<double>(now-uiBenchmarkStart).count()>=uiBenchmarkSeconds){writeUiBenchmarkReport();DestroyWindow(window);}return 0;
  }
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, LPWSTR commandLine, int show) {
  ...
  presentationThread=std::jthread([window](std::stop_token stop){auto next=std::chrono::steady_clock::now();while(!stop.stop_requested()){const int target=std::clamp(requestedFps.load(),30,360);next+=std::chrono::duration_cast<std::chrono::steady_clock::duration>(std::chrono::duration<double>(1.0/target));PostMessageW(window,PresentMessage,0,0);std::this_thread::sleep_until(next);if(std::chrono::steady_clock::now()-next>std::chrono::milliseconds(100))next=std::chrono::steady_clock::now();}});
