...
std::vector<int32_t> renderOwners;
std::unordered_map<int,int> renderSpeciesByOwner;
...
life::AtlasSnapshot renderAtlas;
uint64_t renderAtlasRevision=UINT64_MAX;
BITMAPINFO bitmap{};
...
enum class Overlay { Terrain, Resources, Productivity, Climate, Species };
Tool activeTool = Tool::Inspect;
Overlay activeOverlay = Overlay::Terrain;
bool atlasOpen=false;
int atlasTab=0,atlasTimelineWindow=600;
...
void renderAtlasView(life::skia::Api::Canvas canvas,life::skia::Api::Paint paint,float left,float top,float right,float bottom){
  rectangle(canvas,paint,left,top,right,bottom,0xff121d29);text(canvas,paint,"EVOLUTION ATLAS",left+24,top+34,20,0xffffffff,true);text(canvas,paint,std::to_string(renderAtlas.timeline.size())+" samples · "+std::to_string(renderAtlas.species.size())+" species · "+std::to_string(renderAtlas.events.size())+" events",left+24,top+55,10,0xff81d2c7);
  constexpr const char* tabs[]={"Timeline","Species","Tree","Ecology"};const float tabTop=top+68,tabWidth=(right-left-48)/4;for(int index=0;index<4;++index){const float x=left+24+index*tabWidth;rectangle(canvas,paint,x,tabTop,x+tabWidth-6,tabTop+32,index==atlasTab?0xff81d2c7:0xff3a4b68);text(canvas,paint,tabs[index],x+12,tabTop+21,11,index==atlasTab?0xff121d29:0xffffffff,true);}
  const float bodyTop=tabTop+48;if(atlasTab==0){rectangle(canvas,paint,left+24,bodyTop,right-24,bottom-30,0xff1b2938);if(renderAtlas.timeline.empty()){text(canvas,paint,"Timeline begins after 60 simulation ticks.",left+42,bodyTop+42,12,0xff9099c2);return;}const size_t count=std::min<size_t>(renderAtlas.timeline.size(),std::max(10,atlasTimelineWindow)),begin=renderAtlas.timeline.size()-count;int maximum=1;for(size_t index=begin;index<renderAtlas.timeline.size();++index)maximum=std::max(maximum,renderAtlas.timeline[index].population);const float graphLeft=left+52,graphRight=right-42,graphTop=bodyTop+34,graphBottom=bottom-78;float previousX=graphLeft,previousY=graphBottom;for(size_t offset=0;offset<count;++offset){const auto& sample=renderAtlas.timeline[begin+offset];const float x=graphLeft+(graphRight-graphLeft)*offset/std::max<size_t>(1,count-1),y=graphBottom-(graphBottom-graphTop)*sample.population/maximum;if(offset){const int steps=std::max(1,int(std::abs(x-previousX)));for(int step=0;step<=steps;++step){const float amount=float(step)/steps,px=previousX+(x-previousX)*amount,py=previousY+(y-previousY)*amount;rectangle(canvas,paint,px,py,px+2,py+2,0xff81d2c7);}}previousX=x;previousY=y;}const auto& latest=renderAtlas.timeline.back();text(canvas,paint,"Population "+std::to_string(latest.population)+"  Peak "+std::to_string(maximum)+"  O2 "+std::to_string(int(latest.oxygen*100))+"%  Species "+std::to_string(latest.species),graphLeft,bodyTop+20,11,0xffffffff);text(canvas,paint,"Wheel zooms timeline · recent "+std::to_string(count)+" samples",graphLeft,bottom-48,10,0xff9099c2);int eventLine=0;for(auto event=renderAtlas.events.rbegin();event!=renderAtlas.events.rend()&&eventLine<3;++event,++eventLine)text(canvas,paint,std::to_string(event->tick)+" · "+event->label,graphLeft,bottom-30-eventLine*16,9,0xffd7bd52);}
  else if(atlasTab==1){text(canvas,paint,"ID    Population   Peak   Parent   Diet   State",left+32,bodyTop+18,10,0xff81d2c7,true);int row=0;for(const auto& species:renderAtlas.species){if(row>=int((bottom-bodyTop-35)/22))break;const float y=bodyTop+32+row*22;rectangle(canvas,paint,left+24,y-15,right-24,y+5,species.id==renderAtlas.selectedSpecies?0xff416788:(row%2?0xff1b2938:0xff202d3b));std::ostringstream value;value<<"#"<<species.id<<"      "<<species.population<<"          "<<species.peakPopulation<<"      "<<(species.parentId<0?0:species.parentId)<<"       "<<int(species.dietMask)<<"     "<<(species.extinct?"Extinct":"Living");text(canvas,paint,value.str(),left+34,y,10,species.extinct?0xff9099c2:0xffffffff);++row;}}
  else if(atlasTab==2){text(canvas,paint,"Species ancestry",left+32,bodyTop+18,12,0xff81d2c7,true);int row=0;for(const auto& species:renderAtlas.species){if(row>=int((bottom-bodyTop-30)/20))break;const float y=bodyTop+42+row*20;const int depth=species.parentId<0?0:1;text(canvas,paint,std::string(depth*4,' ')+(species.parentId>=0?"↳ ":"")+"Species #"+std::to_string(species.id)+"  from #"+std::to_string(std::max(0,species.parentId))+(species.extinct?" †":""),left+40,y,10,species.extinct?0xff9099c2:0xffffffff);++row;}}
  else {if(renderAtlas.timeline.empty()){text(canvas,paint,"Ecology sampling begins after 60 ticks.",left+40,bodyTop+38,12,0xff9099c2);return;}const auto& sample=renderAtlas.timeline.back();const std::array<std::string,8> lines{"Population "+std::to_string(sample.population)+" · Living species "+std::to_string(sample.species),"Resources  Plant "+std::to_string(sample.plant)+" · Carrion "+std::to_string(sample.carrion)+" · Mineral "+std::to_string(sample.mineral),"Diets  Plant "+std::to_string(sample.plantEaters)+" · Scavenger "+std::to_string(sample.scavengers)+" · Mineral "+std::to_string(sample.mineralEaters),"Predators "+std::to_string(sample.predators)+" · Critical respiration "+std::to_string(sample.criticalRespiratory),"Average energy "+std::to_string(int(sample.averageEnergy))+" · Body "+std::to_string(sample.averageBodySize)+" · Perception "+std::to_string(sample.averagePerceptionCost),"Atmosphere  O2 "+std::to_string(sample.oxygen*100)+"% · CO2 "+std::to_string(sample.carbonDioxide*100)+"%","Climate  Temperature "+std::to_string(sample.temperature)+" · Fertility "+std::to_string(sample.fertility),"Latest event  "+(renderAtlas.events.empty()?std::string("None"):renderAtlas.events.back().label)};for(size_t index=0;index<lines.size();++index)text(canvas,paint,lines[index],left+40,bodyTop+40+index*30,index==0?13:11,index==0?0xffffffff:0xffb8c3d1,index==0);}
}
...
void render(HWND window, HDC dc) {
  ...
  if(atlasOpen||activeOverlay==Overlay::Species){std::shared_lock atlasLock(simulationMutex,std::try_to_lock);if(atlasLock.owns_lock()){if(renderAtlasRevision!=simulation.atlasRevision()){renderAtlas=simulation.atlasSnapshot();renderAtlasRevision=renderAtlas.revision;renderSpeciesByOwner.clear();renderSpeciesByOwner.reserve(simulation.organisms().size());for(const auto& organism:simulation.organisms())if(organism.living)renderSpeciesByOwner[organism.id]=organism.speciesId;}if(activeOverlay==Overlay::Species)renderOwners=simulation.owners;}}
  ...
  top=585;rectangle(canvas,paint,panelX,top,panelRight,float(height-15),0xffe1e3ec);text(canvas,paint,"Evolution Observatory",panelX+20,top+35,16,0xff121d29,true);rectangle(canvas,paint,panelRight-62,top+13,panelRight-20,top+42,0xff9099c2);text(canvas,paint,"Clear",panelRight-56,top+32,9,0xff111111,true);if(inspection){std::ostringstream identity,energy,health,brain,place,neural,mutation;identity<<"#"<<inspection->id<<" · "<<inspection->diet<<" · "<<(inspection->alive?"Alive":"Dead");if(!inspection->alive&&!inspection->deathCause.empty())identity<<" ("<<inspection->deathCause<<")";energy<<"Energy "<<std::fixed<<std::setprecision(1)<<inspection->energy/1000.0<<"  Minerals "<<inspection->minerals<<"  Age "<<inspection->age<<"  Body "<<inspection->cellCount<<"  Damage "<<inspection->damage;health<<"Resp "<<std::setprecision(1)<<inspection->respiratoryStress*100<<"%  Risk "<<inspection->respiratoryRisk*100<<"%  O2 tol "<<inspection->oxygenTolerance*100<<"%  Recovery "<<inspection->stressRecovery*100<<"%  Frailty "<<inspection->frailty;brain<<"Radius "<<inspection->perceptionRadius<<"  Channels 0x"<<std::hex<<int(inspection->senseChannels)<<std::dec<<"  Cost "<<inspection->neuralCost;place<<inspection->terrain<<" · "<<inspection->resource<<"  Temp "<<std::fixed<<std::setprecision(2)<<inspection->temperature<<"  Fertility "<<inspection->fertility<<"  Thermal "<<inspection->thermalStress*100<<"%";neural<<"Action "<<inspection->action;if(inspection->hasOutputs){neural<<" [";for(int i=0;i<life::OutputSize;++i){if(i)neural<<", ";neural<<std::setprecision(2)<<inspection->outputs[i];}neural<<"]";}mutation<<"Mutations "<<inspection->mutations.size()<<": "<<(inspection->mutations.empty()?"None":inspection->mutations.back());const std::array<std::string,10> lines{identity.str(),energy.str(),health.str(),brain.str(),place.str(),"Ancestors: "+lineageIds(inspection->ancestors),"Descendants: "+lineageIds(inspection->descendants),"Senses: "+featureIds(inspection->senses),neural.str(),mutation.str()};for(size_t line=0;line<lines.size();++line)text(canvas,paint,lines[line],panelX+20,top+62+float(line)*20,line==0?12:9,line==0?0xff121d29:0xff3a4b68,line==0);}else{text(canvas,paint,"Select a creature.",panelX+20,top+68,12,0xff66738a);}
  std::ostringstream populationSummary,performanceSummary;populationSummary<<"Plant "<<metrics.plantEaters<<" · Carrion "<<metrics.scavengers<<" · Mineral "<<metrics.mineralEaters<<" · Predators "<<metrics.predators;performanceSummary<<"O2 "<<std::fixed<<std::setprecision(1)<<metrics.oxygen*100<<"% · Resp "<<metrics.averageRespiratoryStress*100<<"% · Critical "<<metrics.criticalRespiratory<<" · Resp deaths "<<metrics.respiratoryDeaths;rectangle(canvas,paint,panelX,float(height-61),panelRight,float(height-15),0xff121d29);text(canvas,paint,populationSummary.str(),panelX+10,float(height-42),9,0xffffffff);text(canvas,paint,performanceSummary.str(),panelX+10,float(height-24),9,0xff81d2c7);
  rectangle(canvas,paint,24,float(height-52),320,float(height-20),0xee121d29);text(canvas,paint,"Terrain + resources + evolving organisms",38,float(height-31),11,0xffffffff);if(atlasOpen)renderAtlasView(canvas,paint,0,headerHeight,contentRight,float(height));
...
LRESULT CALLBACK procedure(HWND window, UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  if(message==WM_KEYDOWN&&(wParam=='A'||wParam=='a')){atlasOpen=!atlasOpen;InvalidateRect(window,nullptr,FALSE);return 0;}
  if (message == WM_TIMER) {
  ...
  if (message == WM_LBUTTONDOWN || message == WM_MBUTTONDOWN) {
    ...
    if(message==WM_LBUTTONDOWN&&atlasOpen&&mouseX<contentRight&&mouseY>=150){const float tabWidth=(contentRight-48)/4.0f;if(mouseY<=182){atlasTab=std::clamp(int((mouseX-24)/tabWidth),0,3);}else if(atlasTab==1&&mouseY>=212){const int row=(mouseY-212)/22;if(row>=0&&row<int(renderAtlas.species.size())){const auto species=renderAtlas.species[size_t(row)];std::unique_lock lock(simulationMutex);simulation.selectSpecies(species.id);selectedId=species.representativeId;simulation.select(selectedId);}}InvalidateRect(window,nullptr,FALSE);return 0;}
    if(message==WM_LBUTTONDOWN&&mouseY>=25&&mouseY<=58&&mouseX>=contentRight-210&&mouseX<=contentRight-116){atlasOpen=!atlasOpen;InvalidateRect(window,nullptr,FALSE);return 0;}
    if(message==WM_LBUTTONDOWN&&mouseX>=contentRight){
      ...
      else if(mouseY>=337&&mouseY<=363&&mouseX>=panelX+82&&mouseX<=panelRight-20){activeOverlay=Overlay((int(activeOverlay)+1)%5);}
      else if(mouseY>=372&&mouseY<=407&&mouseX>=panelX+20&&mouseX<=panelRight-20){const float half=(panelRight-panelX-45)/2;if(mouseX<=panelX+20+half&&MessageBoxW(window,L"Reset life and clear the Evolution Atlas history?",L"Life Engine",MB_ICONWARNING|MB_OKCANCEL)!=IDOK)return 0;std::unique_lock lock(simulationMutex);if(mouseX<=panelX+20+half)simulation.reset();else simulation.regenerate(nextWorldSeed++);selectedId=-1;simulation.select(-1);}
  ...
  if (message == WM_MOUSEWHEEL) {
    ...
    RECT client{};GetClientRect(window,&client);const float aside=std::min(390.0f,client.right*.32f),contentRight=client.right-aside;if(atlasOpen&&cursor.x<contentRight){atlasTimelineWindow=std::clamp(atlasTimelineWindow+(GET_WHEEL_DELTA_WPARAM(wParam)>0?-120:120),120,24000);InvalidateRect(window,nullptr,FALSE);return 0;}
