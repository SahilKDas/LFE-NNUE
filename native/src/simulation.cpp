...
void NativeSimulation::updateOrganism(Organism& organism) {
  ...
  const double oxygenDeficit=std::max(0.0,.12-atmosphericOxygen_),co2Excess=std::max(0.0,carbonDioxide_-.07),respiratoryLoad=oxygenDeficit*.10+co2Excess*.05;organism.hypoxiaStress=std::clamp(organism.hypoxiaStress+(respiratoryLoad>0?respiratoryLoad:-.01),0.0,1.0);
