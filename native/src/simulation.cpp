...
void NativeSimulation::pruneLineage() {
  ...
  activeSlots_.clear();for(size_t slot=0;slot<organisms_.size();++slot)if(organisms_[slot].living){organisms_[slot].activeIndex=activeSlots_.size();activeSlots_.push_back(slot);}
}
...
void NativeSimulation::updateOrganism(Organism& organism) {
  ...
  const double oxygenDeficit=std::max(0.0,.16-atmosphericOxygen_),co2Excess=std::max(0.0,carbonDioxide_-.05),respiratoryLoad=oxygenDeficit*.22+co2Excess*.12;organism.hypoxiaStress=std::max(0.0,organism.hypoxiaStress+(respiratoryLoad>0?respiratoryLoad:-.004));if(organism.hypoxiaStress>=1){organism.hypoxiaStress-=1;if(mortalityEnabled_){++organism.damage;if(organism.damage>=int(organism.cells.size())){die(organism);return;}}}
  ...
  if(reproductionEnabled_&&atmosphericOxygen_>=.12&&carbonDioxide_<=.10&&(!populationTarget_||int(activeSlots_.size())<populationTarget_)&&organism.energy>=int(organism.cells.size())*6*EnergyScale)reproduce(organism);
