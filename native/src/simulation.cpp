...
const char* deathCauseName(DeathCause cause){constexpr const char* names[]={"","Starvation","Combat","Old age","Thermal stress","Terrain","Respiratory failure"};const auto index=size_t(cause);return index<std::size(names)?names[index]:"Unknown";}
...
NativeSimulation::NativeSimulation(int width, int height, double foodChance, int lifespan, uint32_t worldSeed, uint32_t randomSeed, int lineageLimit)
  : width_(width), height_(height), worldSeed_(worldSeed), random_(randomSeed), foodChance_(foodChance), lifespan_(lifespan),lineageLimit_(std::max(0,lineageLimit)),
    climateTemperature_(height),climateFertility_(height),climateGradient_(height),climateBand_(height),world(generateWorld(width, height, worldSeed)), cells(width * height), owners(width * height, -1) {regionColumns_=(width_+RegionSize-1)/RegionSize;regionRows_=(height_+RegionSize-1)/RegionSize;dirtyColumns_=(width_+DirtyTileSize-1)/DirtyTileSize;dirtyRows_=(height_+DirtyTileSize-1)/DirtyTileSize;regionBatches_.resize(regionColumns_*regionRows_);regionAwake_.resize(regionBatches_.size());dirtyTiles_.resize(dirtyColumns_*dirtyRows_,1);dirtyVersions_.resize(dirtyTiles_.size(),1);organisms_.reserve(100'000);reset(); }
...
int NativeSimulation::wrappedIndex(int x,int y) const{return wrapY(y)*width_+wrapX(x);}
void NativeSimulation::markDirtyIndex(int index){if(index<0)return;const int x=index%width_,y=index/width_,tile=(y/DirtyTileSize)*dirtyColumns_+x/DirtyTileSize;dirtyTiles_[tile]=1;++dirtyVersions_[tile];}
...
void NativeSimulation::placeBody(const Organism& organism) {
  for (const auto& cell : organism.cells) { const auto [rx,ry]=rotated(cell.x,cell.y,organism.rotation);const int index=wrappedIndex(organism.x+rx,organism.y+ry);cells[index]=uint8_t(cell.type);owners[index]=organism.id;markDirtyIndex(index); }
}
void NativeSimulation::clearBody(const Organism& organism){for(const auto& cell:organism.cells){const auto [rx,ry]=rotated(cell.x,cell.y,organism.rotation);const int index=wrappedIndex(organism.x+rx,organism.y+ry);if(owners[index]==organism.id){cells[index]=uint8_t(CellType::Empty);owners[index]=-1;markDirtyIndex(index);}}}
...
int NativeSimulation::seedBenchmarkMovers(int count) {
  ...
  reproductionEnabled_=mortalityEnabled_=true;populationTarget_=count;births_=respiratoryDeaths_=0;lastRespiratoryDeathTick_=-10;
  ...
  for(int y=2;y<height_-2&&int(organisms_.size())<count;y+=3)for(int x=2;x<width_-2&&int(organisms_.size())<count;x+=3,++candidate){
    ...
    largest_=std::max(largest_,int(organism.cells.size()));organism.activeIndex=activeSlots_.size();activeSlots_.push_back(organisms_.size());organisms_.push_back(std::move(organism));slotById_[organisms_.back().id]=organisms_.size()-1;placeBody(organisms_.back());
  }
  record_=int(organisms_.size());updateClimateCache();markAllDirty();rebuildRegionIndex();return int(organisms_.size());
}

bool NativeSimulation::paintTerrain(int x, int y, TerrainType terrain) {
  const int index = safeIndex(x, y);
  if (index < 0) return false;
  if(owners[index]>=0&&(terrain==TerrainType::Water||terrain==TerrainType::Mountain))if(auto* organism=organismById(owners[index]))die(*organism,DeathCause::Terrain);
...
void NativeSimulation::pruneLineage() {
  ...
  std::vector<int> activeIds;activeIds.reserve(activeSlots_.size());for(const size_t slot:activeSlots_)activeIds.push_back(slot<organisms_.size()&&organisms_[slot].living?organisms_[slot].id:-1);
  ...
  activeSlots_.clear();for(const int id:activeIds){if(id<0){activeSlots_.push_back(SIZE_MAX);continue;}const auto found=slotById_.find(id);if(found==slotById_.end()){activeSlots_.push_back(SIZE_MAX);continue;}organisms_[found->second].activeIndex=activeSlots_.size();activeSlots_.push_back(found->second);}
  rebuildRegionIndex();
}

void NativeSimulation::eat(Organism& organism, CellType mouth, int x, int y) {
  for(const auto [dx,dy]:directions){const int index=wrappedIndex(x+dx,y+dy);const auto resource=ResourceType(world.resources[index]);
...
void NativeSimulation::produce(Organism& organism, int x, int y) {
  ...
  const auto [dx,dy]=directions[int(std::floor(random_()*4.0))];const int index=wrappedIndex(x+dx,y+dy);if(TerrainType(world.terrain[index])!=TerrainType::Water&&TerrainType(world.terrain[index])!=TerrainType::Mountain){organism.energy-=ProduceCost;world.resources[index]=uint8_t(ResourceType::Plant);world.resourceAmount[index]=uint16_t(std::min(65535,int(world.resourceAmount[index])+50));markDirtyIndex(index);}
}
...
BodyCell* NativeSimulation::localCellAt(Organism& organism,int x,int y){for(auto& cell:organism.cells){const auto [rx,ry]=rotated(cell.x,cell.y,organism.rotation);if(wrapX(organism.x+rx)==wrapX(x)&&wrapY(organism.y+ry)==wrapY(y))return &cell;}return nullptr;}
void NativeSimulation::harm(Organism& organism){++organism.damage;if(mortalityEnabled_&&organism.damage>=int(organism.cells.size()))die(organism,DeathCause::Combat);}
void NativeSimulation::harmAt(Organism& organism,int index){if(CellType(cells[index])==CellType::Armor){auto* armor=localCellAt(organism,index%width_,index/width_);if(armor&&armor->durability){--armor->durability;if(armor->durability<=0){armor->type=CellType::Inert;cells[index]=uint8_t(CellType::Inert);markDirtyIndex(index);refreshCapabilities(organism);}return;}}harm(organism);}
void NativeSimulation::attack(Organism& attacker,BodyCell& weapon,int x,int y){if(!weapon.durability)return;for(const auto& [dx,dy]:directions){const int index=wrappedIndex(x+dx,y+dy);if(CellType(cells[index])==CellType::Armor)continue;auto* victim=organismById(owners[index]);if(!victim||victim==&attacker||!victim->living)continue;const bool mutual=CellType(cells[index])==CellType::Killer;if(attacker.energy<AttackCost)return;attacker.energy-=AttackCost;--weapon.durability;const int before=victim->damage;harmAt(*victim,index);if(victim->damage>before)attacker.energy+=240;if(mutual)harm(attacker);if(weapon.durability<=0){weapon.type=CellType::Inert;const int origin=wrappedIndex(x,y);cells[origin]=uint8_t(CellType::Inert);markDirtyIndex(origin);refreshCapabilities(attacker);return;}}}
bool NativeSimulation::isClear(const Organism& organism,int x,int y,int rotation) const {
  for(const auto& cell:organism.cells){const auto [rx,ry]=rotated(cell.x,cell.y,rotation);const int index=wrappedIndex(x+rx,y+ry);const auto terrain=TerrainType(world.terrain[index]);if(terrain==TerrainType::Water||terrain==TerrainType::Mountain)return false;if(owners[index]!=organism.id&&CellType(cells[index])!=CellType::Empty)return false;}return true;
}
bool NativeSimulation::straightPath(int x1,int y1,int x2,int y2,const Organism& parent) const {
  const auto passable=[&](int x,int y){const int index=wrappedIndex(x,y);const auto terrain=TerrainType(world.terrain[index]);return terrain!=TerrainType::Water&&terrain!=TerrainType::Mountain&&(CellType(cells[index])==CellType::Empty||CellType(cells[index])==CellType::Food||owners[index]==parent.id);};
...
void NativeSimulation::features(const Organism& organism,std::vector<uint16_t>& result) const {
  result.clear();if(result.capacity()<64)result.reserve(64);int square=0;
  for(int ring=1;ring<=MaximumSensorRadius;++ring)for(int dy=-ring;dy<=ring;++dy)for(int dx=-ring;dx<=ring;++dx)if((dx||dy)&&std::max(std::abs(dx),std::abs(dy))==ring){const int current=square++;if(std::max(std::abs(dx),std::abs(dy))>organism.perceptionRadius)continue;const int index=wrappedIndex(organism.x+dx,organism.y+dy);int category=6,owner=-1;if(index>=0){const auto type=CellType(cells[index]);owner=owners[index];const Organism* neighbor=owner>=0&&owner!=organism.id?organismById(owner):nullptr;category=owner==organism.id?3:type==CellType::Killer?5:neighbor&&neighbor->isMover?7+neighbor->direction:owner>=0?4:type==CellType::Food?1:type==CellType::Wall?2:0;}
  ...
  const int row=std::clamp(organism.y,0,height_-1);if(mask&Temperature){result.push_back(uint16_t(climateOffset+climateBand_[row]));result.push_back(uint16_t(climateOffset+5+climateGradient_[row]+1));}if(mask&Fertility){result.push_back(uint16_t(climateOffset+8+int(std::floor(cyclePhase(ticks_)*4.0))%4));result.push_back(uint16_t(climateOffset+12+std::min(4,int(std::floor((climateFertility_[row]-.35)*5.0)))));}
}
bool NativeSimulation::attemptMove(Organism& organism){const auto [dx,dy]=directions[organism.direction];const int nextX=wrapX(organism.x+dx),nextY=wrapY(organism.y+dy);if(!isClear(organism,nextX,nextY,organism.rotation))return false;clearBody(organism);organism.x=nextX;organism.y=nextY;placeBody(organism);return true;}
...
void NativeSimulation::reproduce(Organism& parent) {
  ...
  if(geneRoll(0x5245434f56455259ULL)%100<12){child.stressRecovery=std::clamp(child.stressRecovery+geneDelta(0x202)*.004,.002,.025);child.mutations.push_back("Respiratory recovery changed to "+std::to_string(child.stressRecovery));}
  if(geneRoll(0x465241494c545931ULL)%100<12){child.frailty=std::clamp(child.frailty+geneDelta(0x303)*.20,.35,2.0);child.mutations.push_back("Frailty changed to "+std::to_string(child.frailty));}
...
void NativeSimulation::die(Organism& organism,DeathCause cause){if(!organism.living)return;organism.deathTick=ticks_;organism.deathCauseCode=cause;organism.deathCause=deathCauseName(cause);++deadCount_;++deathsByCause_[size_t(cause)];if(organism.speciesId>0)for(auto& species:species_)if(species.id==organism.speciesId){species.recentDeathCauses[species.recentDeathCursor]=uint8_t(cause);species.recentDeathCursor=uint8_t((species.recentDeathCursor+1)%species.recentDeathCauses.size());species.recentDeathCount=uint8_t(std::min<size_t>(species.recentDeathCount+1,species.recentDeathCauses.size()));break;}for(const auto& cell:organism.cells){const auto [rx,ry]=rotated(cell.x,cell.y,organism.rotation);const int index=wrappedIndex(organism.x+rx,organism.y+ry);if(owners[index]==organism.id){cells[index]=uint8_t(CellType::Empty);owners[index]=-1;world.resources[index]=uint8_t(ResourceType::Carrion);world.resourceAmount[index]=uint16_t(std::min(65535,int(world.resourceAmount[index])+200+organism.energy/std::max(1,int(organism.cells.size()))/20));markDirtyIndex(index);}}organism.living=false;if(organism.activeIndex<activeSlots_.size())activeSlots_[organism.activeIndex]=SIZE_MAX;}
void NativeSimulation::processRespiratoryMortality(){if(!mortalityEnabled_||ticks_-lastRespiratoryDeathTick_<10)return;respiratoryCandidates_.clear();respiratoryCandidates_.reserve(std::max(respiratoryCandidates_.capacity(),activeSlots_.size()));for(const size_t slot:activeSlots_){if(slot==SIZE_MAX||slot>=organisms_.size())continue;const auto& organism=organisms_[slot];if(organism.living&&organism.respiratoryRisk>=.05)respiratoryCandidates_.push_back({organism.respiratoryRisk,organism.id});}if(respiratoryCandidates_.empty())return;const auto lowerPriority=[](const RespiratoryCandidate& left,const RespiratoryCandidate& right){return left.risk<right.risk||(left.risk==right.risk&&left.id>right.id);};std::make_heap(respiratoryCandidates_.begin(),respiratoryCandidates_.end(),lowerPriority);if(auto* victim=organismById(respiratoryCandidates_.front().id)){die(*victim,DeathCause::RespiratoryFailure);++respiratoryDeaths_;lastRespiratoryDeathTick_=ticks_;}}
void NativeSimulation::updateOrganism(Organism& organism) {
  ...
  const int climateRow=wrapY(organism.y);const double temperature=climateTemperature_[climateRow];const int home=wrappedIndex(organism.x,organism.y);const bool desert=TerrainType(world.terrain[home])==TerrainType::Desert;
  ...
  if(organism.thermalStress>=1){organism.thermalStress-=1;if(mortalityEnabled_){++organism.damage;if(organism.damage>=int(organism.cells.size())){die(organism,DeathCause::ThermalStress);return;}}}
  const double co2Tolerance=.07+(.12-organism.oxygenTolerance)*.5,oxygenDeficit=std::max(0.0,organism.oxygenTolerance-atmosphericOxygen_),co2Excess=std::max(0.0,carbonDioxide_-co2Tolerance);const double agePressure=1.0+.25*std::clamp(double(organism.lifetime)/std::max(1.0,double(organism.cells.size()*lifespan_)),0.0,1.0),energyPressure=organism.energy<int(organism.cells.size())*2*EnergyScale?1.35:1.0,temperaturePressure=1.0+discomfort,bodyPressure=1.0+.04*std::log2(std::max(1.0,double(organism.cells.size()))),respiratoryLoad=(oxygenDeficit*.10+co2Excess*.05)*agePressure*energyPressure*temperaturePressure*bodyPressure;organism.hypoxiaStress=std::clamp(organism.hypoxiaStress+(respiratoryLoad>0?respiratoryLoad:-organism.stressRecovery),0.0,1.0);organism.respiratoryRisk=std::max(0.0,organism.hypoxiaStress-.65)*organism.frailty*agePressure*energyPressure;
  if(mortalityEnabled_&&organism.lifetime>int(organism.cells.size())*lifespan_){die(organism,DeathCause::OldAge);return;}
  if(organism.energy<=0){organism.energy=0;if(mortalityEnabled_){++organism.damage;if(organism.damage>=int(organism.cells.size())){die(organism,DeathCause::Starvation);return;}}}
  const double oxygenHealth=std::clamp((atmosphericOxygen_-.10)/.08,0.0,1.0),carbonHealth=std::clamp((.10-carbonDioxide_)/.06,0.0,1.0),atmosphereHealth=std::min(oxygenHealth,carbonHealth);const uint64_t reproductionDraw=(uint64_t(organism.id)*0x9e3779b97f4a7c15ULL+uint64_t(ticks_)*0xbf58476d1ce4e5b9ULL)>>54;const bool atmosphereAllowsBirth=reproductionDraw<uint64_t(std::round(atmosphereHealth*atmosphereHealth*1023.0));
  if(reproductionEnabled_&&atmosphereAllowsBirth&&(!populationTarget_||int(activeSlots_.size())<populationTarget_)&&organism.energy>=int(organism.cells.size())*6*EnergyScale)reproduce(organism);
...
GenomeDescriptor NativeSimulation::describeGenome(const Organism& organism) const {
  GenomeDescriptor result;result.brainGenomeId=organism.brainGenomeId;result.neuralSignature=organism.brain?organism.brain->signature():0;result.perceptionRadius=uint8_t(organism.perceptionRadius);result.senseChannels=organism.senseChannels;result.moveRange=uint16_t(std::clamp(organism.moveRange,0,65535));result.birthDistance=uint16_t(std::clamp(organism.birthDistance,0,65535));result.oxygenTolerance=uint16_t(std::round(organism.oxygenTolerance*10'000));result.stressRecovery=uint16_t(std::round(organism.stressRecovery*10'000));result.frailty=uint16_t(std::round(organism.frailty*1'000));result.mutability=uint16_t(std::clamp(organism.mutability,0,65535));result.neuralMutability=uint16_t(std::clamp(int(std::round(organism.neuralMutability*100)),0,65535));
  for(const auto& cell:organism.cells){const int type=std::clamp(int(cell.type),0,11);if(result.cellCounts[type]<255)++result.cellCounts[type];if(cell.type==CellType::Mouth||cell.type==CellType::PlantMouth)result.dietMask|=1;else if(cell.type==CellType::ScavengerMouth)result.dietMask|=2;else if(cell.type==CellType::MineralMouth)result.dietMask|=4;}
  uint64_t canonical=UINT64_MAX;std::array<uint64_t,256> encoded{};const size_t count=std::min(encoded.size(),organism.cells.size());for(int rotation=0;rotation<4;++rotation){int minX=INT_MAX,minY=INT_MAX;for(size_t index=0;index<count;++index){const auto [x,y]=rotated(organism.cells[index].x,organism.cells[index].y,rotation);minX=std::min(minX,x);minY=std::min(minY,y);}for(size_t index=0;index<count;++index){const auto [x,y]=rotated(organism.cells[index].x,organism.cells[index].y,rotation);encoded[index]=(uint64_t(uint8_t(organism.cells[index].type))<<32)|(uint64_t(uint16_t(x-minX))<<16)|uint16_t(y-minY);}std::sort(encoded.begin(),encoded.begin()+count);uint64_t hash=1469598103934665603ULL;for(size_t index=0;index<count;++index){hash^=encoded[index];hash*=1099511628211ULL;}canonical=std::min(canonical,hash);}result.bodyHash=canonical;return result;
}
void NativeSimulation::addEvolutionEvent(EvolutionEventType type,int speciesId,float value,const std::string& label){if(evolutionEvents_.size()>=20'000)evolutionEvents_.erase(evolutionEvents_.begin(),evolutionEvents_.begin()+1'000);evolutionEvents_.push_back({type,ticks_,speciesId,value,label});++atlasRevision_;}
void NativeSimulation::classifySpecies(){
...
void NativeSimulation::step(int count) {
  for(int iteration=0;iteration<count;++iteration){
    ++ticks_;updateClimateCache();const size_t activeCount=activeSlots_.size();const int deathsBefore=deadCount_;
    for(size_t index=0;index<activeCount;++index){
      const size_t slot=activeSlots_[index];if(slot==SIZE_MAX||slot>=organisms_.size()||!organisms_[slot].living)continue;const size_t before=organisms_.size();updateOrganism(organisms_[slot]);
    ...
    processRespiratoryMortality();if(deadCount_!=deathsBefore){activeSlots_.erase(std::remove(activeSlots_.begin(),activeSlots_.begin()+activeCount,SIZE_MAX),activeSlots_.begin()+activeCount);for(size_t index=0;index<activeSlots_.size();++index)organisms_[activeSlots_[index]].activeIndex=index;}
    updateAtmosphere();if((ticks_&7)==0||deadCount_!=deathsBefore||activeSlots_.size()!=activeCount)rebuildRegionIndex();
    if(deadCount_>lineageLimit_)pruneLineage();record_=std::max(record_,int(activeSlots_.size()));if(ticks_%300==0)classifySpecies();if(ticks_%60==0)sampleEcology();
  }
}
NativeMetrics NativeSimulation::metrics() const {
  double energy=0,mutation=0,stress=0,oxygenTolerance=0,respiratoryStress=0;int living=0,plantEaters=0,scavengers=0,mineralEaters=0,predators=0,criticalRespiratory=0;
  ...
  for(size_t region=0;region<regionBatches_.size();++region){if(!regionAwake_[region])continue;for(const size_t slot:regionBatches_[region]){const auto& organism=organisms_[slot];energy+=organism.energy;mutation+=organism.mutability;stress+=organism.thermalStress;oxygenTolerance+=organism.oxygenTolerance;respiratoryStress+=organism.hypoxiaStress;criticalRespiratory+=organism.respiratoryRisk>=.05;++living;memory+=organism.cells.capacity()*sizeof(BodyCell)+organism.mutations.capacity()*sizeof(std::string)+organism.lastFeatures.capacity()*sizeof(uint16_t);for(const auto& cell:organism.cells){if(cell.type==CellType::Mouth||cell.type==CellType::PlantMouth)++plantEaters;else if(cell.type==CellType::ScavengerMouth)++scavengers;else if(cell.type==CellType::MineralMouth)++mineralEaters;else if(cell.type==CellType::Killer)++predators;}}}
  const auto climate=climateAt(height_/2,height_,ticks_);const int awake=int(std::count(regionAwake_.begin(),regionAwake_.end(),uint8_t(1))),dirty=int(std::count(dirtyTiles_.begin(),dirtyTiles_.end(),uint8_t(1)));return{living,record_,resets_,ticks_,largest_,living?energy/living/EnergyScale:0,living?mutation/living:0,living?stress/living:0,climate.temperature,climate.fertility,atmosphericOxygen_,carbonDioxide_,living?oxygenTolerance/living:0,living?respiratoryStress/living:0,plantEaters,scavengers,mineralEaters,predators,nnueEvaluations_,respiratoryDeaths_,criticalRespiratory,memory,awake,int(regionAwake_.size())-awake,dirty};
}
}
