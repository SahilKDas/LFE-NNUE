...
NativeSimulation::NativeSimulation(int width, int height, double foodChance, int lifespan, uint32_t worldSeed, uint32_t randomSeed)
  : width_(width), height_(height), worldSeed_(worldSeed), random_(randomSeed), foodChance_(foodChance), lifespan_(lifespan),
    world(generateWorld(width, height, worldSeed)), cells(width * height), owners(width * height, -1) { reset(); }
...
void NativeSimulation::updateOrganism(Organism& organism) {
  ...
  if(organism.lifetime>int(organism.cells.size())*lifespan_){organism.living=false;return;}
...
void NativeSimulation::step(int count) { for(int iteration=0;iteration<count;++iteration){++ticks_;for(auto& organism:organisms_)updateOrganism(organism);record_=std::max(record_,int(organisms_.size()));} }
