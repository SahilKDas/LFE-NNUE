...
bool NativeSimulation::paintTerrain(int x, int y, TerrainType terrain) {
  ...
  if(owners[index]>=0&&(terrain==TerrainType::Water||terrain==TerrainType::Mountain))if(auto* organism=organismById(owners[index]))die(*organism,"Terrain");
...
void NativeSimulation::harm(Organism& organism){++organism.damage;if(mortalityEnabled_&&organism.damage>=int(organism.cells.size()))die(organism,"Combat");}
...
void NativeSimulation::updateOrganism(Organism& organism) {
  ...
  if(organism.thermalStress>=1){organism.thermalStress-=1;if(mortalityEnabled_){++organism.damage;if(organism.damage>=int(organism.cells.size())){die(organism,"Thermal stress");return;}}}
  ...
  if(mortalityEnabled_&&organism.lifetime>int(organism.cells.size())*lifespan_){die(organism,"Old age");return;}
  if(organism.energy<=0){organism.energy=0;if(mortalityEnabled_){++organism.damage;if(organism.damage>=int(organism.cells.size())){die(organism,"Starvation");return;}}}
